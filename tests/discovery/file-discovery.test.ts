...
describe('src/discovery/file-discovery.ts', () => {
  ...
  it('discovers default files into categories', async () => {
    ...
    expect(res.svelteModules).toEqual(expect.arrayContaining([
      expect.stringMatching(/module\.svelte\.ts$/),
      expect.stringMatching(/bar\.svelte\.js$/),
    ]))

    // typescript: .ts files that are not .svelte.ts
    expect(res.typescript).toEqual(expect.arrayContaining([
      expect.stringMatching(/util\.ts$/),
    ]))

    // javascript: plain .js files
    expect(res.javascript).toEqual(expect.arrayContaining([
      expect.stringMatching(/signal\.js$/),
    ]))
  ...
  it('custom include patterns add files beyond defaults', async () => {
    ...
    const opts: DiscoveryOptions = {
      include: ['**/*.svelte', '**/*.ts', '**/*.js', '**/*.svelte.ts', '**/*.svelte.js', '**/*.jsx'],
    }
  ...
  it('edge case: .svelte.ts and .svelte.js go to svelteModules', async () => {
    ...
    expect(res.typescript).not.toEqual(expect.arrayContaining([expect.stringMatching(/module\.svelte\.ts$/)]))
    expect(res.javascript).not.toEqual(expect.arrayContaining([expect.stringMatching(/bar\.svelte\.js$/)]))
  })
})
